...
contract Flashloan {
    ...
    function pancakeV3Flashcallback(
        ...
        //trading 1 swapping base for first token
        acquiredAmount = _placeSwap(
            acquiredAmount,
        ...
        //trading 2 swap token for token
        acquiredAmount = _placeSwap(
            acquiredAmount,
        ...
        //Repay the Flashloan and caller
        if (fee0 > 0) {
            ...
            token1.safeTransfer(address(pool), decoded.amount1 + fee0); //pay flashloan
            console.log("Token1: ", address(token1));
            console.log("Token1 Balance: ", token1.balanceOf(address(this)));
            token0.safeTransfer(decoded.caller, token1.balanceOf(address(this))); //pay caller
   ...
   constructor(address _token0, address _token1, uint24 _fee){
    ...
    function checkBalances(address token, uint256 amount) internal view returns (bool) {
    ...
    function _placeSwap(
        ...
        if (_routing == 0) {
           // address routerV2 = address(pancakeswapV2);
